...
export default class QuillCursors {
  ...
  update() {
    Object.keys(this.cursors).map((key) => {
  ...
  _shiftCursor(userId, index, length) {
    var cursor = this.cursors[userId];
  ...
  _updateCursor(cursor) {
    ...
    if (
      ...
      console.warn(
        "[quill-cursors] A cursor couldn't be updated (ID " +
          cursor.userId +
          '), hiding.'
      );
  ...
  _updateSelection(cursor, rects, containerRect) {
    const createSelectionBlock = (rect) => {
      ...
      selectionBlockEl.style.backgroundColor = tinycolor(cursor.color)
        .setAlpha(0.3)
        .toString();
