...
var cursors = {};
cursors.socket = new ReconnectingWebSocket(
  (window.location.protocol === 'https:' ? 'wss' : 'ws') + '://' + window.location.hostname + ':4000/cursors'
);
...
cursors.socket.onmessage = function (message) {
  ...
  if (!cursors.localConnection.id) forceUpdate = true;
  ...
  for (var i = 0; i < cursors.connections.length; i++) {
    ...
    if (!testConnection) {
      ...
      if (data.sourceId == cursors.connections[i]) source = cursors.connections[i];
  ...
  for (var i = 0; i < data.connections.length; i++) {
    ...
    if (data.sourceId == data.connections[i].id) source = data.connections[i];

    if (
      reportNewConnections &&
      !cursors.connections.find(function (connection) {
  ...
  document.dispatchEvent(
    new CustomEvent('cursors-update', {
      detail: {
        source: source,
